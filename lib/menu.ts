...
export function generateDay(date:string,catalog:Dish[],s:Settings,history:Day[],old?:Day,random= Math.random):Day {
 const people=peopleFor(s);const day:Day={date,locked:false,meals:emptyMeals(s)};
...
export function menuWarnings(menu:Menu,history:Day[]){const people=peopleFor(menu.settings);const warnings:string[]=[];let repeats=0;const seen=[...history];for(const day of menu.days){for(const p of people)for(const dish of day.meals[p]??[])if(seen.some(h=>distance(day.date,h.date)<repeatInterval(dish,menu.settings)&&h.meals[p]?.some(x=>x.id===dish.id||normalize(x.name)===normalize(dish.name))))repeats++;seen.push(day)}if(repeats)warnings.push(`Hay ${repeats} asignaciones repetidas antes de la separación orientativa de su complejidad. Es una preferencia; puedes cambiar esos días.`);if(menu.days.some(d=>people.some(p=>d.meals[p]?.some(x=>x.review))))warnings.push('El menú incluye platos marcados para revisión en vuestro Excel. Comprueba sus datos antes de confirmar.');return warnings}
