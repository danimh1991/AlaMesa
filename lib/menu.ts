...
export type Day = {date:string;meals:Record<Person,Dish[]>;mealsByType?:Partial<Record<MealType,Record<Person,Dish[]>>>;locked:boolean;manual?:ManualMeal;suggestion?:string;suggestedRecipe?:import('./recipes').Recipe;personal?:Record<string,PersonalMeal>};
...
export function generateMenu(month:string,catalog:Dish[],settings:Settings,menus:Record<string,Menu>,previous?:Menu):Menu{
 ...
 for(const date of monthDates(month,settings.days)){const saved=locked.find(d=>d.date===date);if(saved){if(peopleFor(previous!.settings).join()!==peopleFor(settings).join())throw new Error('Han cambiado los comensales. Desbloquea los días antes de regenerar este mes.');const keep=structuredClone(saved);if(!keep.manual)for(const type of selectedMealTypes(settings)){const handled=type==='Comida'&&(!!keep.suggestion||Object.keys(keep.personal??{}).length>0);if(!handled&&peopleFor(settings).some(p=>!(mealsFor(keep,type)[p]??[]).length)){const generated=generateDay(date,catalog,{...settings,mealTypes:[type]},[...history,...locked.filter(d=>d.date!==date),...days],keep);setMealsFor(keep,type,mealsFor(generated,type));}}validateDay(keep,settings);days.push(keep)}else days.push(generateDay(date,catalog,settings,[...history,...locked,...days.filter(d=>!d.locked)],previous?.days.find(d=>d.date===date)))}
