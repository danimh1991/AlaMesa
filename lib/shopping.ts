...
import {normalize,peopleFor,manualEntries,selectedMealTypes,mealsFor,personalFor,type Dish,type RecipeDetails,type Menu} from './menu';
...
export function shoppingList(menu:Menu,catalog:Dish[],from=menu.month+'-01',to=menu.month+'-31'){
 ...
 for(const type of selectedMealTypes(menu.settings))for(const [person,meal] of Object.entries(personalFor(day,type))){if(meal.kind==='custom'||meal.kind==='tupper')issue('personal:'+day.date+':'+type+':'+person,meal.note||'Tupper',type+' puntual: comprueba si necesita compra.',day.date);}
 const uses=new Map<number,{dish:Dish;count:number}>();for(const type of selectedMealTypes(menu.settings))for(const p of peopleFor(menu.settings).filter(p=>!personalFor(day,type)[p]))for(const snapshot of mealsFor(day,type)[p]??[]){const old=uses.get(snapshot.id);if(old)old.count++;else uses.set(snapshot.id,{dish:catalog.find(d=>d.id===snapshot.id)??snapshot,count:1})}
