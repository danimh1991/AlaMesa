...
export function shoppingList(menu:Menu,catalog:Dish[],from=menu.month+'-01',to=menu.month+'-31'){
 ...
 const uses=new Map<number,{dish:Dish;count:number}>();for(const p of peopleFor(menu.settings).filter(p=>!day.personal?.[p]))for(const snapshot of day.meals[p]??[]){const old=uses.get(snapshot.id);if(old)old.count++;else uses.set(snapshot.id,{dish:catalog.find(d=>d.id===snapshot.id)??snapshot,count:1})}
