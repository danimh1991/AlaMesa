...
import {normalize,people,type Dish,type RecipeDetails,type Menu} from './menu';
...
export function shoppingList(menu:Menu,catalog:Dish[],from=menu.month+'-01',to=menu.month+'-31'){
 ...
 if(day.manual){if(day.manual.kind==='custom'){for(const text of new Set([day.manual.Dani,day.manual.Marta]))issue('manual:'+text,text,'Comida puntual: comprueba si necesita compra.',day.date);}continue;}
 const uses=new Map<number,{dish:Dish;count:number}>();for(const p of people)for(const snapshot of day.meals[p]){const old=uses.get(snapshot.id);if(old)old.count++;else uses.set(snapshot.id,{dish:catalog.find(d=>d.id===snapshot.id)??snapshot,count:1})}
 for(const {dish,count} of uses.values()){portions+=count;const r=recipeFor(dish);if(!r.reviewed||!r.servings||!r.ingredients.length){issue('dish:'+dish.id,dish.name,'Faltan ingredientes, raciones o revisión de la receta.',day.date,dish.id);continue;}let complete=true;
