...
export function planImport(document:unknown,catalog:Dish[],highestReservedId=0){const parsed=catalogDocumentSchema.safeParse(document);if(!parsed.success)throw new Error(parsed.error.issues.slice(0,6).map(i=>`${i.path.join('.')}: ${i.message}`).join('\n'));
 const ids=new Set<number>(),newNames=new Set<string>();let next=Math.max(highestReservedId,...catalog.map(d=>d.id),0)+1;const changes:{dish:Dish;kind:'new'|'updated'|'unchanged'}[]=[];
