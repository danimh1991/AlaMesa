...
import {initialState,normalizeDish,normalizeSettings,type Day,type Dish,type State} from './menu';
...
function normalizeMeals(day:Day){for(const [person,dishes] of Object.entries(day.meals))day.meals[person]=dishes.map(d=>normalizeDish(d));}
function normalizeState(state:State){
 ...
 for(const menu of Object.values(state.menus)){menu.settings=normalizeSettings(menu.settings);for(const day of menu.days)normalizeMeals(day)}
 return state;
}
