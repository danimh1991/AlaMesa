...
import {generateDay,generateMenu,allowed,normalize,peopleFor,emptyMeals,monthDates,manualEntries,dinersFor,selectedMealTypes,mealsFor,setMealsFor,type Menu,type Dish,type State,type Day,type RecipeDetails} from './menu';
...
function eligible(pool:Recipe[],catalog:Dish[],date:string,menu:Menu,person=peopleFor(menu.settings)[0]){return pool.filter(r=>!catalog.some(d=>d.recipeId===r.id||normalize(d.name)===normalize(r.name))&&allowed({...r,id:0,diners:peopleFor(menu.settings),mealType:'Comida',review:false},person,date,menu.settings,'Comida'));}
...
export function acceptRecipe(state:State,catalog:Dish[],body:{recipeId:string;dish:Omit<Dish,'id'|'recipeId'>;month?:string;date?:string;schedule?:boolean;replaceLocked?:boolean;targetPerson?:string},recipe:Recipe){
 ...
 if(body.month&&body.date){
  ...
  if(target&&dish.mealType!=='Comida')throw new Error('La edición puntual por comensal solo admite recetas de comida.');
...
export function suggestForDay(menu:Menu,catalog:Dish[],date:string,pool:Recipe[]=pilot){
 ...
 const recipe=options[Math.floor(Math.random()*options.length)],replacement=structuredClone(day);Object.assign(replacement,{locked:true,suggestion:recipe.id,suggestedRecipe:recipe});setMealsFor(replacement,'Comida',emptyMeals(menu.settings));delete replacement.manual;delete replacement.personal;menu.days[index]=replacement;menu.recipeSlots=[...new Set([...(menu.recipeSlots??[]),date])];
}
export function editPerson(menu:Menu,catalog:Dish[],menus:State['menus'],body:{date:string;person:string;mode:'out'|'tupper'|'custom'|'generate'|'suggest';note?:string},pool:Recipe[]){
 ...
 if(body.mode==='generate'){
  ...
  const settings={...menu.settings,mealTypes:['Comida' as const],diners:dinersFor(menu.settings).filter(d=>d.id===p)};day.meals[p]=mealsFor(generateDay(body.date,catalog,settings,history,day),'Comida')[p];setMealsFor(day,'Comida',day.meals);delete day.personal![p];
