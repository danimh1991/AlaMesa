...
import {generateDay,allowed,normalize,peopleFor,emptyMeals,type Menu,type Dish,type State,type Day} from './menu';
...
export function addSuggestions(menu:Menu,catalog:Dish[],menus:Record<string,Menu>,previous?:Menu){
 ...
 const eligible=pilot.filter(r=>!catalog.some(d=>d.recipeId===r.id||normalize(d.name)===normalize(r.name))&&!alreadyTried.has(r.id)&&allowed({...r,id:0,person:'Ambos',review:false},peopleFor(menu.settings)[0],menu.month+'-01',menu.settings));
 ...
 day.suggestion=recipe.id;day.locked=true;day.meals=emptyMeals(menu.settings);day.manual={kind:'custom',entries:Object.fromEntries(peopleFor(menu.settings).map(p=>[p,recipe.name]))};menu.recipeSlots.push(day.date);
 }
 return menu;
}
export function acceptRecipe(state:State,catalog:Dish[],body:{recipeId:string;dish:Omit<Dish,'id'|'recipeId'>;month?:string;date?:string}){
 ...
 replacement={date:day.date,locked:true,meals:emptyMeals(menu!.settings)};
 for(const p of peopleFor(menu.settings)){if(dish.person!=='Ambos'&&dish.person!==p){replacement.meals[p]=generateDay(day.date,catalog,menu.settings,history).meals[p];continue;}
 if(!allowed(dish,p,day.date,menu.settings))throw new Error('La temporada elegida no encaja con este mes.');
 if(dish.type==='Único')replacement.meals[p]=[dish];else{const candidates=[{...dish,person:'Ambos'},...catalog.filter(d=>d.person==='Ambos'||d.person===p).filter(d=>d.type===(dish.type==='Entrante'?'Principal':'Entrante')).map(d=>({...d,person:'Ambos'}))];replacement.meals[p]=generateDay(day.date,candidates,menu.settings,history).meals[p].map(d=>d.id===dish.id?dish:catalog.find(x=>x.id===d.id)!);}
 }
 }
...
export function suggestForDay(menu:Menu,catalog:Dish[],date:string){
 ...
 const eligible=pilot.filter(r=>!used.has(r.id)&&!catalog.some(d=>d.recipeId===r.id||normalize(d.name)===normalize(r.name))&&allowed({...r,id:0,person:'Ambos',review:false},peopleFor(menu.settings)[0],date,menu.settings));
 ...
 menu.days[index]={date,locked:true,suggestion:recipe.id,meals:emptyMeals(menu.settings),manual:{kind:'custom',entries:Object.fromEntries(peopleFor(menu.settings).map(p=>[p,recipe.name]))}};
