...
const input=z.discriminatedUnion('action',[
 ...
 z.object({action:z.literal('person-day'),month,date:z.string().regex(/^20\d{2}-\d{2}-\d{2}$/),person:z.string().min(1).max(80),mealType:z.enum(['Desayuno','Comida','Merienda','Cena']).default('Comida'),mode:z.enum(['out','tupper','custom','generate','suggest']),note:z.string().trim().max(240).optional(),revision:z.number().int().min(0)}),
