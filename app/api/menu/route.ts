...
export async function POST(request:Request){
 ...
 else if(body.action==='preview-import'||body.action==='import-catalog'){const plan=planImport(body.document,catalog);if(body.action==='preview-import')return json({preview:plan.summary,revision});applyImport(state,plan);if(catalogFor(state).some(d=>!Object.keys(state.settings.categoryRanges).some(category=>normalize(category)===normalize(d.category))))throw new Error('La importación contiene tipos principales que no están en Preferencias. Añádelos antes de importar.')}
